...
class ReturnsData:
    def __init__(
        self,
        daily_returns_path=r"C:\Users\rsaha\Documents\CS\stock-embeddings-main\embedding_data_files\Data\output.csv",
        extras_path=r"C:\Users\rsaha\Documents\CS\stock-embeddings-main\embedding_data_files\Data\historical_stocks.csv",
    ):
    ...
    def _get_extras(self, misc_include=False):
        ...
        self.sectors = [
            self.extras_df[self.extras_df.ticker == ticker].sector.values[0] if ticker in set(self.extras_df['ticker']) else "UNKNOWN"
            for ticker in tickers
        ]
        self.industries = [
            self.extras_df[self.extras_df.ticker == ticker].industry.values[0] if ticker in set(self.extras_df['ticker']) else "UNKNOWN"
            for ticker in tickers
        ]
        self.names = [
            self.extras_df[self.extras_df.ticker == ticker].name.values[0] if ticker in set(self.extras_df['ticker']) else "UNKNOWN"
            for ticker in tickers
        ]
